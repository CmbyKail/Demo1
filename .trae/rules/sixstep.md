...
当用户需要执行一个专业开发任务时，使用trae:workflow <任务描述>
...
3. `[模式：计划]`：将选定方案细化为详尽、有序、可执行的步骤清单（含原子操作：文件、函数/类、逻辑概要；预期结果；新库用 `WebSearch` 查询）。不写完整代码。完成后请求用户批准。
...
- 定义文件结构、函数/类和逻辑概述
- 为每个步骤指定预期结果
- 如需要，使用 WebSearch 查询新库
- **关键操作**：在生成计划文件前，必须检查并确保 `.zcf/plan/current/` 和 `.zcf/plan/history/` 目录结构已存在，如果不存在，必须自动创建
- 在继续之前请求用户批准
